...
module.exports = async function () {
  ...
  function convertToId(subjectName) {
    const matchingSubjects = subjects.filter(s => s.name === subjectName);

    if (matchingSubjects.length === 0) throw Error("Matched no subject, check fixtures.");
    if (matchingSubjects.length > 1) throw Error("Matched more than one subject, check fixtures.");
